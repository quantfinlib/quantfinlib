...
def chu_stinchcombe_white_cusum_test(y: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Compute the Chu-Stinchcombe-White CUSUM test for structural breaks.

    We compute the standardized departure of the log-price y_t relative to the log-price
    at y_n (t > n) as follows:

    .. math::
        S_{n,t} = \\frac{y_t - y_n}{\hat{\\sigma_t} \\sqrt{t - n - 1}}

    where the estimated variance :math:`\hat{\\sigma_t}` is calculated as:
    ...
    We compute S_t as the supremum of S_{n,t} over a backward-shifting window,
    n ∈ [1, t]:
